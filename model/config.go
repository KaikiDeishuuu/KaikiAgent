...
type AgentConfig struct {
	...
	// Standalone 模式：启用后 Agent 不会尝试连接哪吒面板 (server/client_secret 可为空)
	Standalone                  bool            `koanf:"standalone" json:"standalone"`
...
func ValidateConfig(c *AgentConfig, isRemoteEdit bool) error {
	...
	if !isRemoteEdit {
		// 如果启用了 Standalone 模式，则允许 server 和 client_secret 为空
		if !c.Standalone {
			if c.Server == "" {
